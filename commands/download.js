...
cli.command(
	...
	"Download the specific release version of kb2abot-client & kb2abot-dedicated-server",
	yargs => {
	...
	async argv => {
		...
		const index =
			ver != "latest" ? releases.findIndex(rl => rl.name == ver) : 0

		if (index == -1 || releases.length == 0) return error("Release not found!")
		...
		fs.renameSync(
			path.join(process.cwd(), `kb2ateam-kb2abot-${commit.sha.slice(0, 7)}`),
			dest
		)
