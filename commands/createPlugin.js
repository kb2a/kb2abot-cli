...
cli.command(
	"plugin <plugin-name>",
	"Create folder plugin including example command",
	yargs => {
	...
	argv => {
		...
		fs.writeFileSync(path.join(plgDir, "src", "index.js"), Plugin(name, argv))
		fs.writeFileSync(
			path.join(plgDir, "src", "commands", "example.js"),
			Command("example", argv)
		)
		fs.writeFileSync(path.join(plgDir, "package.json"), Package(name, argv))
